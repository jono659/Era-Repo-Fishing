...
function onGameIn(player, firstlogin, zoning)
    ...
    if ((player:getHP() == 0) and (player:getZone():getID() == 43)) then
    player:warp();
    end
	...
	if (player:getVar("MoogleBuffs") == 1) then
		...
		if (player:getVar("MentorFlag") == 1) then
			...
			player:delStatusEffect(EFFECT_STR_BOOST); -- Mentor Only
			player:delStatusEffect(EFFECT_HASTE); -- Mentor Only
...
function CharCreate(player)
   ...
   ----- settings.lua Perks -----
	...
	if (isValidLS("TehSocial") == true) then -- Make sure LS exists
		player:addLS("TehSocial"); -- Give an LS pearl to all new players
	end
