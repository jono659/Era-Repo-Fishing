...
function onUseAbility(player,target,ability)
    ...
    local AV = GetMobByID(16912876);
		printf("Time: %i | benedictionTime: %i | Diff: %i", os.time(), AV:getLocalVar("benedictionTime"), os.time() - AV:getLocalVar("benedictionTime"));
		if AV:isSpawned() and (os.time()-1) - AV:getLocalVar("benedictionTime") < 5 then
			if (player:checkDistance(AV) <= 20) and (partyType == 2) then
				AV:setLocalVar("benediction", 0);
				printf("locked bene");
            end
        end;
    end;
