...
require("scripts/globals/scoreboard");
...
function getMultiAttacks(attacker, numHits)
    ...
    for i = 1, multiChances, 1 do
        ...
        if (i == 1) then
            attacker:delStatusEffect(dsp.effects.ASSASSIN_S_CHARGE);
...
function applyAftermathEffect(player, tp, params)
    ...
    local apply_power = 0
    if (tp == 3000 and shouldApplyAftermath(player, dsp.effects.AFTERMATH_LV3)) then
        player:delStatusEffect(dsp.effects.AFTERMATH_LV1);
        player:delStatusEffect(dsp.effects.AFTERMATH_LV2);
        player:addStatusEffect(dsp.effects.AFTERMATH_LV3, params.power.lv3, 0,
            params.duration.lv3, 0, params.subpower.lv3);
    elseif (tp >= 2000 and shouldApplyAftermath(player, dsp.effects.AFTERMATH_LV2)) then
        player:delStatusEffect(dsp.effects.AFTERMATH_LV1);
        apply_power = math.floor(params.power.lv2 + ((tp - 2000) / (100 / params.power.lv2_inc)))
        player:addStatusEffect(dsp.effects.AFTERMATH_LV2, apply_power, 0,
            params.duration.lv2, 0, params.subpower.lv2);
    elseif (tp >= 1000 and shouldApplyAftermath(player, dsp.effects.AFTERMATH_LV1)) then
        apply_power = math.floor(params.power.lv1 + ((tp - 1000) / (100 / params.power.lv1_inc)))
        player:addStatusEffect(dsp.effects.AFTERMATH_LV1, apply_power, 0,
...
function shouldApplyAftermath(player, effect)
    local result = true;
    if (effect == dsp.effects.AFTERMATH_LV1 and (player:hasStatusEffect(dsp.effects.AFTERMATH_LV2) or player:hasStatusEffect(dsp.effects.AFTERMATH_LV3))) then
        result = false;
    elseif (effect == dsp.effects.AFTERMATH_LV2 and player:hasStatusEffect(dsp.effects.AFTERMATH_LV3)) then
        result = false;
    end;
