...
function onUseWeaponSkill(player, target, wsID, tp, primary, action, taChar)
	if (player:getVar("MythicBlm") >= 500) then
    ...
    if (damage > 0) then
        ...
        if (target:hasStatusEffect(dsp.effects.MAGIC_DEF_DOWN) == false) then
            target:addStatusEffect(dsp.effects.MAGIC_DEF_DOWN, 10, 0, duration);
        end
    end
	local chance = 100
