            ...
            for bcnmindex = 1, #itemid_bcnmid_map[zoneindex + 1], 2 do -- loop bcnms in this zone
                if (trade:getItemId()==itemid_bcnmid_map[zoneindex+1][bcnmindex]) then
                    ...
                    elseif (item == 1174 and player:getVar("CarbuncleDebacleProgress") == 6) then -- Carbuncle Debacle (Ogmios)
                        questTimelineOK = 1
                    elseif ((item == 2108) and (player:getQuestStatus(JEUNO,STORMS_OF_FATE) == QUEST_COMPLETED)) then -- the Wyrmking Decends
                        questTimelineOK = 1
                    elseif ((item == 1842) and (player:hasCompletedMission(COP, ANCIENT_VOWS))) then -- Ouryu Cometh
                        questTimelineOK = 1
                    end
...
function checkNonTradeBCNM(player, npc, mode)
    ...
    local checks =
    {
        ...
        [36] = {
					...
					[1057] = function() return (player:getQuestStatus(JEUNO,APOCALYPSE_NIGH) == QUEST_ACCEPTED) and (player:getVar("ApocalypseNigh")==4) end,-- ApocalypseNigh
               },
        [57] = {
