...
function updateInstanceTime(instance, elapsed, texttable)
    ...
    if (remainingTimeLimit < 0) then
        instance:fail();
		printf("Instance Fail due to time");
        return;
    end
--    printf("timeRemaining: %u",remainingTimeLimit);
--    printf("wipe time: %u",wipeTime);
--    printf("Elapsed time: %u",elapsed);
    if (wipeTime == 0) then
        local wipe = false;
        ...
        if (elapsed - wipeTime) / 1000 > 180 then
            instance:fail();
			printf("instance failed due to wipe");
            return;
        else
            for i,v in pairs(players) do
                if v:getHP() ~= 0 then
