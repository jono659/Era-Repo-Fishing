...
function BluePhysicalSpell(caster, target, spell, params)
	...
	--caster:PrintToPlayer(string.format("aoe %d", checkaoe));
	
    while (hitsdone < params.numhits) do
	local isSneakValid = caster:hasStatusEffect(dsp.effects.SNEAK_ATTACK); 
        local chance = math.random();
		
				
			if (isSneakValid and not (caster:isBehind(target) or caster:hasStatusEffect(dsp.effects.HIDE))) then
				isSneakValid = false;
			end
        ...
        if ((chance <= hitrate) or (isSneakValid)) then -- it hit
					if spell:getID() == 643 then 
						finaldmg = params.offcratiomod*.5;
					end
			...
			if (spell:isAoE() == 0 and isSneakValid) then
					...
					caster:delStatusEffectsByFlag(EFFECTFLAG_DETECTABLE);
				...
				if (hitsdone == 0) then
					...
					--caster:PrintToPlayer(string.format("old pdif is "..pdif));
					pdif = math.random(300,325);
					pdif = pdif / 100;
				
					--caster:PrintToPlayer(string.format("new pdif is "..pdif));
            ...
            if (hitsdone == 0) then -- only the first hit benefits from multiplier
				...
				--finaldmg = finaldmg + params.offcratiomod*0.5;
				--caster:PrintToPlayer(string.format(" hitsdone is 0 finaldmg %d finalD %d pdif %d", finaldmg, finalD, pdif));
            else
			--caster:PrintToPlayer(string.format(" hitsdone is above 0 finaldmg %d finalD %d pdif %d", finaldmg, finalD, pdif));
...
-- caster:PrintToPlayer(string.format(dmg));
    if (dmg < 0) then
        dmg = 0;
    end
...
function BluecRatio(ratio,atk_lvl,def_lvl)
	...
	ccritratio[1] = cratiomin;
	ccritratio[2] = cratiomax;
	
    return cratio, ccritratio;
end;
...
function getBlueEffectDuration(caster,resist,effect)
    ...
    elseif (effect == dsp.effects.WEIGHT) then
        duration = math.random(100,120) + resist * 9; -- 20-24
