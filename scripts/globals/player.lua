...
function onGameIn(player, firstlogin, zoning)
	...
	if (player:getVar("MoogleBuffs") == 1) then
		...
		if (player:getVar("MentorFlag") == 1) then
			...
			player:delStatusEffect(dsp.effects.STR_BOOST); -- Mentor Only
			player:delStatusEffect(dsp.effects.HASTE); -- Mentor Only
    ...
    if (player:getVar("GodMode") == 1) then
        ...
        player:addStatusEffect(dsp.effects.SENTINEL,100,0,0);
