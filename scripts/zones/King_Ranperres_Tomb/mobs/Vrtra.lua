...
function onMobDeath(mob, player, isKiller)
    ...
    player:setVar("vrtracount",player:getVar("vrtracount")+1);
	player:addCurrency("cruor", math.random(700,800));
end;

function onMobDespawn(mob)
    local wait = (math.random(259200,432000)); -- 3 to 5 days
    local Vrtra = mob:getID();

    UpdateNMSpawnPoint(Vrtra);
    SetServerVariable("[POP]VRTRA", os.time() + wait);
end;
