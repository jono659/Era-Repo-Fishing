-- Variable TextID   Description text

-- General Texts
ITEM_CANNOT_BE_OBTAINED = 6380; -- You cannot obtain the item <item> come back again after sorting your inventory
          ITEM_OBTAINED = 6386; -- Obtained: <item>
           GIL_OBTAINED = 6387; -- Obtained <number> gil
       KEYITEM_OBTAINED = 6389; -- Obtained key item: <keyitem>
 FISHING_MESSAGE_OFFSET = 7229; -- You can't fish here.

-- Shop Texts
    LOKHONG_SHOP_DIALOG = 7334; -- There's nothing like fishing to pass the time!
     CHHAYA_SHOP_DIALOG = 7335; -- May I offer you items to help you on your journey?

 -- Other
       ON_WAY_TO_MHAURA = 7329; -- We're on our way to Mhaura
   ARRIVING_SOON_MHAURA = 7336; -- We will be arriving soon
