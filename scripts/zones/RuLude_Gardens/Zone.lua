...
function onZoneIn(player,prevZone)
    ...
    if (player:getXPos() == 0 and player:getYPos() == 0 and player:getZPos() == 0) then
        position = math.random(1,5) + -60;
        player:setPos(position,6,-1.01,261);
        ...
        elseif (player:getCurrentMission(COP) == DAWN) then
            ...
            elseif (player:getVar("PromathiaStatus") == 7) then
                ...
                elseif (player:getQuestStatus(JEUNO,STORMS_OF_FATE) == QUEST_ACCEPTED and player:getVar('StormsOfFate') == 3) then
					...
					elseif ((player:getQuestStatus(JEUNO,SHADOWS_OF_THE_DEPARTED) == QUEST_AVAILABLE) and (player:getQuestStatus(JEUNO,STORMS_OF_FATE) == QUEST_COMPLETED) and player:getVar("ShadowsDepartedDay")~=VanadielDayOfTheYear()) then
                    player:startEvent(161);
				elseif (player:getQuestStatus(JEUNO,SHADOWS_OF_THE_DEPARTED) == QUEST_ACCEPTED and (player:hasKeyItem(PROMYVION_HOLLA_SLIVER) == true) and (player:hasKeyItem(PROMYVION_DEM_SLIVER) == true) and (player:hasKeyItem(PROMYVION_MEA_SLIVER) == true)) then
                    player:startEvent(162);
				elseif (player:getQuestStatus(JEUNO,SHADOWS_OF_THE_DEPARTED) == QUEST_COMPLETED) and (player:getQuestStatus(JEUNO,APOCALYPSE_NIGH) == QUEST_AVAILABLE) then
...
function onEventFinish(player,csid,option)
    ...
    elseif (csid == 143) then
		...
		player:setVar("ShadowsDepartedDay",VanadielDayOfTheYear());
	elseif (csid == 161) then
		player:addQuest(JEUNO,SHADOWS_OF_THE_DEPARTED);
		player:setVar('ShadowsOfTheDeparted',1);
		player:addKeyItem(NOTE_WRITTEN_BY_ESHANTARL);
		player:messageSpecial(KEYITEM_OBTAINED,NOTE_WRITTEN_BY_ESHANTARL);
	elseif (csid == 162) then
		player:setVar('ShadowsOfTheDeparted',0);
		player:completeQuest(JEUNO,SHADOWS_OF_THE_DEPARTED);
	elseif (csid == 123) then
		player:addQuest(JEUNO,APOCALYPSE_NIGH);
		player:setVar('ApocalypseNigh',1);
    end
end;
