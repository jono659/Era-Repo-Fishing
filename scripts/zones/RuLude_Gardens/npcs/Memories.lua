...
if (trade:hasItemQty(18600,1) == true and trade:getItemCount() == 1) then
	...
	elseif (trade:hasItemQty(3083,1) and trade:hasItemQty(1407,1) and trade:getItemCount() == 2 and player:hasItem(11289) == false) then
			player:tradeComplete();
			player:addItem(11289); -- Ixion Cloake
			player:messageSpecial(ITEM_OBTAINED, 11289);
...
function onTrigger(player,npc)
	player:PrintToPlayer("Summer's Sky Event is now on.");
	player:PrintToPlayer("Roll up roll up. Test your luck.");
	player:PrintToPlayer("Trade me a Caver's shovel and i'll give you a stone.");
	player:PrintToPlayer("Or be more daring. Trade a skin and a seal. See what you get.");
	player:PrintToPlayer("I wonder what happens when you trade all 4?");
	player:PrintToPlayer("I heard after this event, Daft will RIP.");
end;
