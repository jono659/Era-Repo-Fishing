...
function onTrade(player,npc,trade)
	...
	if(player:getQuestStatus(JEUNO,SHATTERING_STARS) ~= QUEST_AVAILABLE and player:getMainLvl() >= 66) then
		local mJob = player:getMainJob();
		printf("pre CHECK CHECK CHECK");
			if mJob > 15 then
		printf("during CHECK CHECK CHECK");
				 mJob = 5;
		printf("done CHECK CHECK CHECK");
			end
			printf("post CHECK CHECK CHECK");
		if(trade:hasItemQty(1425 + mJob,1) and tradeCount == 1 and player:getVar("maatDefeated") == 0) then
			player:startEvent(0x0040,mJob); -- Teleport to battlefield for "Shattering Stars"
			elseif(player:getVar("maatDefeated") >= 1) then
			player:PrintToPlayer("Maat: Not so fast, I've got something to say to you.");
				
		end
	elseif player:getMainLvl() >= 66 and mJob == 20 and player:levelCap() == 70 then
	
		if(trade:hasItemQty(2557,1) and tradeCount == 1 and player:getVar("maatDefeated") == 0) then
			player:startEvent(0x0040,5); -- Teleport to battlefield for "Shattering Stars"
			elseif(player:getVar("maatDefeated") >= 1) then
			player:PrintToPlayer("Maat: Not so fast, I've got something to say to you.");
	...
	printf("FAILCHECK DODODODODODODODODODDODODODODODOD");
	printf("%d", player:getMainJob());
	end
...
function onTrigger(player,npc)
	...
	local Maatcap = player:getQuestStatus(JEUNO,BEYOND_THE_SUN);
	...
	elseif(shatteringStars == QUEST_AVAILABLE and LvL >= 66 and mJob > 15 and player:levelCap() == 70 and MAX_LEVEL >= 75 and player:getVar("maatDefeated") == 0) then
		--player:startEvent(0x005c,player:getMainJob()); -- Start Quest "Shattering Stars"
		player:PrintToPlayer("Bring me a Scholar's Testimony");
	elseif(shatteringStars == QUEST_AVAILABLE and LvL >= 66 and mJob > 15 and player:levelCap() == 70 and MAX_LEVEL >= 75 and player:getVar("maatDefeated") >= 1) then
	...
	elseif(Maatcap == QUEST_AVAILABLE) then
			player:addQuest(JEUNO,BEYOND_THE_SUN);
    elseif(Maatcap == QUEST_ACCEPTED and player:getVar("maatCap") < 120 and player:getVar("maatDefeated") >= 1) then
        player:setVar("maatDefeated",0);
        player:PrintToPlayer("Maat: You got lucky, you little punk. That wasn't even my final form...");
	elseif(player:getQuestStatus(JEUNO,BEYOND_THE_SUN) == QUEST_ACCEPTED and mJob <= 15 and player:getVar("maatsCap") == 32767) then
		player:startEvent(0x004a); -- Finish Quest "Beyond The Sun"
	elseif(Maatcap == QUEST_ACCEPTED and player:getVar("maatCap") == 120) then
		player:startEvent(0x004a); --  end maat cap quest
...
--printf("RESULT: %u",option);
	...
	elseif(csid == 0x0040 and option == 1) then
			...
			elseif(mJob == 5 or mJob == 6 or mJob == 9 or mJob == 20) then  	player:setPos(-349.899,104.213,-260.150,0,144);
	...
	elseif(csid == 0x004a) then
		...
		player:messageSpecial(ITEM_CANNOT_BE_OBTAINED,15194);
		else
		player:setVar("maatCap",0);
		player:setVar("M_War",0);
		player:setVar("M_Mnk",0);
		player:setVar("M_Whm",0);
		player:setVar("M_Blm",0);
		player:setVar("M_Rdm",0);
		player:setVar("M_Thf",0);
		player:setVar("M_Pld",0);
		player:setVar("M_Drk",0);
		player:setVar("M_Bst",0);
		player:setVar("M_Brd",0);
		player:setVar("M_Rng",0);
		player:setVar("M_Sam",0);
		player:setVar("M_Nin",0);
		player:setVar("M_Drg",0);
		player:setVar("M_Smn",0);
		player:addItem(15194,1);
