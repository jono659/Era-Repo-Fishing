...
function onTrigger(player,npc)
	...
	elseif(CurrentMission == MOON_READING) then
		if(player:getVar("WINDURST92") == 0) then
			player:startEvent(0x0180);
		elseif(player:getVar("WINDURST92") == 1 and player:hasKeyItem(ANCIENT_VERSE_OF_UGGALEPIH) and player:hasKeyItem(ANCIENT_VERSE_OF_ALTEPA) and player:hasKeyItem(ANCIENT_VERSE_OF_ROMAEVE)) then
			player:startEvent(0x0181);
		elseif(player:getVar("WINDURST92") == 3) then
			player:startEvent(0x0182);
		elseif(player:getVar("WINDURST92") == 4) then
...
function onEventFinish(player,csid,option)
    ...
    elseif (csid == 0x0180) then
		player:setVar("WINDURST92",1);
	elseif(csid == 0x0181) then
		player:setVar("WINDURST92",2);
	...
	elseif(csid == 0x0182) then
		player:setVar("WINDURST92",4);
	elseif(csid == 0x0197) then
		--player:setVar("WINDURST92",5);
		--player:setPos(-257,-5,-120,192,239);
		player:setVar("WINDURST92",0);
		player:addItem(183);
