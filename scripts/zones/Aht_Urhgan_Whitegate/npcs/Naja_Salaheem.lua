...
function onTrade(player,npc,trade)
    if (npcUtil.tradeHas(trade, 2163) and player:getVar("Promotion") == 1)then
        player:confirmTrade()
...
function onTrigger(player,npc)
	...
	if (player:getVar("AssaultPromotion") >= 25 and player:hasKeyItem(PFC_WILDCAT_BADGE) == false) then
        player:startEvent(5000,0,0,0,0,0,0,0,0,0);
