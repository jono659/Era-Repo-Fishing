...
namespace charutils
{
    ...
    ************************************************************************/
    ...
    void DistributeExperiencePoints(CCharEntity* PChar, CMobEntity* PMob)
    {
		...
		// Custom ERA code for zeni and cruor
		if (PChar->PParty != nullptr)
		{
			std::vector<CCharEntity*> members;
			// First gather all valid party members
			...
			for (auto PMember : members)
			{
				if (PMember->StatusEffectContainer->HasStatusEffect(EFFECT_SANCTION) && (region >= 28 && region <= 32) && (members.size() <= 6))
				{	
					CMobEntity* ZeniMob = (CMobEntity*)PMob;
					charutils::AddPoints(PMember, "zeni_point", uint32(ZeniMob->GetMLevel() -50));
				}
				CMobEntity* CruorMob = (CMobEntity*)PMob;
				if ((CruorMob->m_Type & MOBTYPE_NOTORIOUS) && (members.size() > 0) && (region != REGION_DYNAMIS && region != REGION_LIMBUS))
				{	
					charutils::AddPoints(PMember, "cruor", uint32(CruorMob->GetMLevel() * 2.5));
				}
			}
		}
		else if (distance(PChar->loc.p, PMob->loc.p) < 100)
		{
			...
			{
				CMobEntity* ZeniMob = (CMobEntity*)PMob;
				charutils::AddPoints(PChar, "zeni_point", uint32(ZeniMob->GetMLevel() -50));
			}
			CMobEntity* CruorMob = (CMobEntity*)PMob;
			if ((CruorMob->m_Type & MOBTYPE_NOTORIOUS) && (region != REGION_DYNAMIS && region != REGION_LIMBUS))
			{
				charutils::AddPoints(PChar, "cruor", uint32(CruorMob->GetMLevel() * 2.5));
			}
		}
        auto pcinzone = 0;
    ...
    ************************************************************************/
    void DelExperiencePoints(CCharEntity* PChar, float retainPercent, uint16 forcedXpLoss)
    {
       ...
       //Custom ERA Code - Start
		if (PChar->allegiance != 1)
			exploss = (uint16)(exploss *(0));
		else{
        if (PChar->getZone() >= 39 && PChar->getZone() <= 42 || PChar->getZone() == 134 || PChar->getZone() == 135 || PChar->getZone() >= 185 && PChar->getZone() <= 189)
			exploss = (uint16)(exploss * (2.00));
		else
		{	
			if (PChar->m_mentorUnlocked)
				exploss = (uint16)(exploss * (1.00));
			else
			{
    ...
    ************************************************************************/
    ...
    void AddExperiencePoints(bool expFromRaise, CCharEntity* PChar, CBaseEntity* PMob, uint32 exp, uint32 baseexp, bool isexpchain)
    {
        ...
        if (!expFromRaise)
        {
            // Custom ERA Code - Begin
            	if (PChar->m_mentorUnlocked)
					exp = (uint32)(exp * 1.00);
				else if (PChar->getZone() >= 39 && PChar->getZone() <= 42 || PChar->getZone() == 134 || PChar->getZone() == 135 || PChar->getZone() >= 185 && PChar->getZone() <= 189)
					exp = (uint32)(exp * 2.00);
				else
			// Custom ERA Code - End
        ...
        if (!expFromRaise)
        {
                ...
                (region >= 33 && region <= 40))
            {
                ...
                {
                    charutils::AddPoints(PChar, "allied_notes", (int32)(exp * 0.025f));
                    PChar->pushPacket(new CConquestPacket(PChar));
                }
            }
            // Era Custom Sigil End
