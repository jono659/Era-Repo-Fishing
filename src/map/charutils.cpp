    ...
    void DelExperiencePoints(CCharEntity* PChar, float retainPercent, uint16 forcedXpLoss)
    {
       ...
       //Custom ERA Code - Start
        ...
        if (PChar->getZone() >= 39 && PChar->getZone() <= 42 || PChar->getZone() == 134 || PChar->getZone() == 135 || PChar->getZone() >= 185 && PChar->getZone() <= 189)
			exploss = exploss * 2.00;
		else
		{	
			if (PChar->m_mentorUnlocked = true)
				exploss = exploss * 1.00;
			else
			{
        ...
        if ((currentExp + exp) >= GetExpNEXTLevel(PChar->jobs.job[PChar->GetMJob()]) && onLimitMode == false)
        {
            ...
            else
            {
                ...
                if (PChar->PParty != nullptr)
                {
                    if (PChar->PParty->GetSyncTarget() == PChar)
                    {
						if (!PChar->StatusEffectContainer->GetStatusEffect(EFFECT_LEVEL_RESTRICTION)) //Custom ERA Code
						{
							PChar->PParty->RefreshSync();
						}
                    }
